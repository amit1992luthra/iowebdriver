...
# Naming Conventions
	-File name and directory name are in Camel case followed by 
...
3. Installing project
	...
	-Open project using any IDE, webStorm IDE is preferable.
	-Install node modules (run command --> npm i or npm install)
	
    

#Run the tests
	- Mention spec file path in configuration file (In framework in common conf file under respective suite capabilities)
	...
	- Run a test suite using batch(.bat) or shell(.sh) file. 
	

#Configuration file
	-Always mention latest chrome driver version before run in configuration file (In Framework mention in common conf file).
	-Create configuration file suite wise and define respective capabilities in the common conf configuration file.
